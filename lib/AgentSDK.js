...
class SDK extends Events {
     ...
     */
    reconnect(dontRegenerateToken, callback = null) {
        ...
        this.csdsClient.getAll((err, domains) => {
            ...
            this._handleLoginAndReconnect(domains, 'Reconnect#Relogin#WS', (err) => {
    ...
    _init(options) {
        ...
        this.on('error', (error, context) => this._handleError(error, context));
        this.on('closed', (data) => this._handleClosedEvents(data));
    }

    _handleClosedEvents(data) {
        switch (data) {
            // Authentication issue
            case 4401:
            case 4407:
            case 1011:
                this.agent.reconnect();     // call our reconnect looper
                break;
            // Non-authentication issue
            default:
                this.agent.reconnect(true); // call our reconnect looper without token generation
                break;
        }
    }
    ...
    _handleError(error, context) {
        ...
        if (context.location.startsWith(CONNECTION_TYPES.REFRESH_SESSION)) {
            this._handleRefreshSessionError(error);
        } else if (context.location.startsWith(CONNECTION_TYPES.RECONNECT) || context.location.startsWith(CONNECTION_TYPES.CONNECT)) {
            this._handleReconnectError(error);
        }
    }

    _handleRefreshSessionError(error) {
        const connectionType = CONNECTION_TYPES.REFRESH_SESSION;
        if (this._retryStates[connectionType]) {
            return;
        }

        if (this._shouldBackOffRetry(error)) {
            this._retryExponentialBackoff(connectionType, this._shouldBackOffRetry, this.refreshSession);
        }
    }

    _handleReconnectError(error) {
        const connectionType = CONNECTION_TYPES.RECONNECT;
        if (this._retryStates[connectionType]) {
            return;
        }

        // 401 errors come from AgentVEP
        if (error && error.code === 401) {
            this.reconnect(false);

        // API errors come from Logins or CSDS, do a reconnect with false
        } else if (this._shouldBackOffRetry(error)) {
            this._retryExponentialBackoff(connectionType, this._shouldBackOffRetry, (callback) => {
                this.reconnect(false, callback);
            });
        } else {
            this.reconnect(true);
        }
    ...
    _shouldBackOffRetry(error) {
        const rateLimited = error && error.code === 429;
        const serviceDown = error && error.code >= 500;
        const anyOtherError = error && error.error;
        return rateLimited || serviceDown || anyOtherError;
    }

    _retryExponentialBackoff(connectionType, determineShouldBackOff, asyncCall, delay = reconnectIntervalMs, attempt = 0) {
        // Clear the timeouts from before
        this._retryStates[connectionType] = true;
        clearTimeout(this._retryTimeouts[connectionType]);

        this._retryTimeouts[connectionType] = setTimeout(() => {
            asyncCall((err) => {
                if (determineShouldBackOff(err) && ++attempt <= reconnectAttempts) {
                    const randomMs = Math.floor(Math.random() * (reconnectIntervalMs - 0 + 1)) + 0;
                    const newDelay = Math.min(Math.exp(reconnectExponent, attempt) + randomMs, 64000);
                    this._retryExponentialBackoff(connectionType, asyncCall, newDelay, attempt);
                } else {
                    // Don't retry
                    this._retryStates[connectionType] = false;
                }
            });
        }, delay);
    }
