...
class SDK extends Events {
                ...
                if (domains.agentVep) {
                    if (!conf.token) {
                        external.login({
                            ...
                            this.agentId = `${conf.accountId}.${data && data.config && data.config.userId}`; //for external use
    ...
    request(type, body, headers, callback) {
        ...
        if (Transformer[type] && Transformer[type].onRequest) {
            body = Transformer[type].onRequest(body);
        }
    ...
    _handleNotification(msg) {
        if (msg && msg.type) {
            if (Transformer[msg.type]) {
                msg.body = Transformer[msg.type](msg.body);
            }
    ...
    _handleResponse(msg) {
        if (msg && msg.type) {
            if (Transformer[msg.type] && Transformer[msg.type].onResponse) {
                msg.body = Transformer[msg.type].onRequest(msg.body);
            }
    ...
    _notifyOutCome(id, data, error) {
        if (this.pendingRequests[id]) {
            if (this.pendingRequests[id].callback) {
                this.pendingRequests[id].callback(error ? data : null, data);
            }
