...
class RequestQueue {
    constructor(options, ...args) {
        this.maxSize = (options && typeof options.maxSize === "number") ? options.maxSize : Number.POSITIVE_INFINITY;
        this._queue = [];
        this._limiter = (args[0] instanceof RateLimiter) ? args[0] : new RateLimiter(...args);
    }
    addRequest(fn) {
        this._enqueue(fn);
        this._limiter.removeTokens(1, () => this._dequeue());
    }
    _enqueue(task)  {
        ...
        if(this._queue.length >= this.maxSize) {
            process.nextTick(()=>task(new Error("Queue size limit reached")));
        }
        else {
...
class SDK extends Events {
    constructor(conf) {
        ...
        const throttleOptions = Object.assign({
            queueSize: 10000,
            requestsInInterval: 1200,
            intervalPeriod: 10000
        }, conf.throttle);
        this._throttleQueue = new RequestQueue({maxSize: throttleOptions.queueSize},  throttleOptions.requestsInInterval, throttleOptions.intervalPeriod);
    ...
    _queueForResponseAndSend(request, callback) {
        const fn = (err) => {
            ...
            else if (request && this.connected) {
        ...
        if(request.type === '.GetClock') {
            fn();
        }
        else {
            this._throttleQueue.addRequest(fn);
        }
