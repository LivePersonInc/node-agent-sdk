...
class SDK extends Events {
    ...
    _retryExponentialBackoff(connectionType, determineShouldBackOff, asyncCall, delay = reconnectInterval, attempt = 1) {
        ...
        this._retryTimeouts[connectionType] = setTimeout(() => {
            asyncCall((err) => {
                if (determineShouldBackOff(err) && ++attempt <= reconnectAttempts) {
                    this._retryExponentialBackoff(connectionType, asyncCall, delay * reconnectRatio, attempt);
                } else {
                    // Don't retry
