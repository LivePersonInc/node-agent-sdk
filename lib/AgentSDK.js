...
class SDK extends Events {
    ...
    _handleRefreshSessionError(error) {
        ...
        if (this.retryStates[connectionType]) {
            return;
        }

        if (this._shouldBackOffRetry(error)) {
            this._retryExponetialBackoff(connectionType, this._shouldBackOffRetry, this.refreshSession);
        }
    }
    ...
    _handleReconnectError(error) {
        ...
        // API errors come from Logins or CSDS, do a reconnect with false
            ...
            this._retryExponetialBackoff(connectionType, this._shouldBackOffRetry, (callback) => {
    ...
    _retryExponetialBackoff(connectionType, determineShouldBackOff, asyncCall, delay = reconnectInterval, attempt = 1) {
        ...
        this._retryTimeouts[connectionType] = setTimeout(() => {
            asyncCall((err) => {
                if (determineShouldBackOff(err) && ++attempt <= reconnectAttempts) {
                    this._retryExponetialBackoff(connectionType, asyncCall, delay * reconnectRatio, attempt);
                } else {
                    // Don't retry
