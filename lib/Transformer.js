        ...
        msg.body.changes.forEach(change => {
            if (change && change.result) {
                ...
                if (!conversationDetails.hasOwnProperty('participantsPId')) {
                    let message = `conversationDetails.participantsPId not found on conversation ${change.result.convId}`;
                    console.warn(message);
                    agent.emit('warn', {message: message, data: change});
                }
                else {
                    conversationDetails.dialogs.forEach(dialog => {
                        ...
                        dialog.participantsDetails.forEach(p => {
                            ...
                            // get all possible pids for this role from the conversation
                            let rolePIds = conversationDetails.participantsPId[p.role];

                            // initialize the roleCount if this is the first we've seen
                            if (!(p.role in roleCount)) roleCount[p.role] = 0;

                            // Grab the index and increment
                            let rolePIdIndex = roleCount[p.role];
                            roleCount[p.role]++;

                            // use the given PID, but only if the index exists
                            if (rolePIds.length > rolePIdIndex) p.id = rolePIds[rolePIdIndex];
                            else {
                                let message =`invalid pid index on ${change.result.convId}`;
                                console.warn(message);
                                agent.emit('warn', {message: message, data: change});
                            }
                        });
